...
export function clean(paths: string|string[]): (done: () => void) => void {
  return done => {
    ...
    let promises = pathsToClean.map(p => {
      return new Promise(resolve => {
        ...
        if (relativePath.startsWith('..')) {
          util.log(util.colors.bgRed.white(
              `Cannot remove files outside the project root but tried "${normalize(p)}"`));
          process.exit(1);
        } else {
          rimraf(p, e => {
            if (e) {
    ...
    Promise.all(promises)
        .then(() => (done || (() => 1))())
        .catch(
            e => util.log(util.colors.red(
                `Error while removing files "${[].concat(paths).join(', ')}", ${e}`)));
  };
}
